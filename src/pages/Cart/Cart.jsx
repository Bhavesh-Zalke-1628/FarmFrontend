...
function Cart() {
    ...
    const handlePurchase = useCallback(() => {
        ...
        if (items.some(item => item.outOfStock || item.isDeleted)) {
    ...
    if (error) {
        return <ErrorCart error={error} onRetry={() => dispatch(fetchCart())} />;
    }
...
const CartItem = React.memo(({ item, onIncrement, onDecrement, onRemove }) => {
    const discountedPrice = calculateDiscountedPrice(item.price, item.offerPercentage, item.quantity);
    const originalPrice = calculateOriginalPrice(item.price, item.quantity);
